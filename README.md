...
独立 Chrome Manifest V3 扩展。在 X 帖子视频右上角提供小型毛玻璃下载按钮，点击可选择最高画质视频（MP4）或最高音质音频（M4A）。视频下方自动显示分辨率、帧率和音频编码、码率、采样率与声道数，无需先下载。当前版本 **1.2.0**。
...
2. 点击「加载已解压的扩展程序」，选择本项目的 `dist` 目录。也可以解压 `release/x-video-downloader-1.2.0.zip`，选择解压后包含 `manifest.json` 的目录。
...
3. 确认扩展版本变为 **1.2.0**，刷新所有已打开的 X 页面。
