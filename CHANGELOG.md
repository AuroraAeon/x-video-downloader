...
- 跨机可复现修复：构建改为把复制出的文本项统一成 LF（Windows 的 `core.autocrlf` 曾让 `manifest.json` 多出 42 字节），打包改为本地零点时间戳（`zipSync` 用本地时间字段写 DOS 时间戳，UTC+8 机器曾写成 08:00 而 CI 是 00:00）。修复后本机在 UTC、America/New_York、Asia/Kolkata、Pacific/Kiritimati 四种时区重打包，SHA-256 都与 CI 发布到 GitHub Release 的 `x-video-downloader-1.3.0.zip` 完全相同（`b23e23ecfe3e3d4e72270d2ec31c625e07a972f6e3211663d9a8602a73ba2469`）；已发布的 1.3.0 产物由 CI 构建，不受影响。两条修复各配常驻门禁：`scripts/check-package.mjs` 拒绝归档文本条目里的 `0x0d` 字节，并逐个解析 ZIP 本地头确认时间戳为 1980-01-01 00:00；两处断言都经注入实测会以退出码 1 拒绝。
