...
1.1.1 将布局定位拆为按钮容器与信息栏文档流锚点。信息栏不再与按钮共用「最多向上五层且尺寸一致」的容器：它越过绝对定位、固定高度、aspect-ratio 和播放器裁剪层，挂在正常内容流中的媒体区域之后、帖子互动栏之前。它通过自身高度推开后续内容，保持播放器原有尺寸，不靠绝对定位或额外顶部偏移伪装成外部信息栏。

ResizeObserver 等待零尺寸播放器就绪，DOM/style/class 变更后重新校正现有 UI 节点；播放器重新挂载时保留数据、菜单监听和任务状态。`scripts/layout-test.mjs` 除了验证信息栏与视频、互动行都不相交，还检查互动行下移的高度与视频尺寸不变。

IntersectionObserver 对视口附近 180px 的视频安排检查，1.2.0 采用两条背景探测队列，最多排队 40 个，真实可见项优先。离开视口取消排队及无人使用的活动探测；显式下载等待的探测保留。元数据不足时详情回退串行执行，每个播放器身份只自动尝试一次。媒体 URL 改变时失效，下载候选缓存两分钟，显示摘要缓存十五分钟；两者写入 storage.session，后台休眠后继续复用。

ProbeQueue 保留当前订阅标签页，后台重启后仍可将结果投递到来源页面。后台和离屏文档分别持有小型信息摘要缓存与媒体候选缓存；未生成的文件不写入 OPFS，也不进入下载任务记录。空闲十五秒后关闭离屏文档。

1.2.0 的探测与正式下载采用不同网络源：探测使用标准 CustomSource 的 32 KiB Range 块，只读取必要头部和样本表；正式下载使用 UrlSource 的流式顺序读取。三个输入有界并行、音视频轨道并行，共享每媒体四个网络槽位。Progressive QualitySummary 标记 pending，不能放入最终候选缓存，也不能显示为已确认最高质量。详细测量与错误预算见 performance.md。
