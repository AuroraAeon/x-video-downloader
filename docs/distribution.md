...
- GitHub 仓库：`AuroraAeon/x-video-downloader`。已完成：八个 topics 与 `homepageUrl`（`gh repo edit` 写入并读回，见第 0 节）、首页顶部的中英双链与"商店尚未上架 + 编号三步本地加载"安装说明、v1.3.0 Release 附 `x-video-downloader-1.3.0.zip` 与 `SHA256SUMS.txt`（由 `.github/workflows/release.yml` 自动构建上传）。仓库描述用与商店一致的说明（英文那份已经写好：`README.en.md` 首段，中英双份首页由 `tests/community.test.ts` 同步校验）。剩下的是首页 `homepageUrl` 指向的地址在 Pages 启用前返回 404。
...
2. 启用 Pages（GitHub → Settings → Pages → Source: GitHub Actions），让 `.github/workflows/pages.yml` 部署 `docs/site`，提供最终的隐私政策 URL。实测现状：`https://auroraeon.github.io/x-video-downloader/`、`/privacy.html`、`/sitemap.xml` 均返回 404，即尚未部署。同一天 `pages.yml` 的首次运行失败并报 `Get Pages site failed … Error: Not Found`，原因就是这个开关没开，不是站点构建问题（站点构建本身成功）。只想改工作流也能开，但要存一个长期凭证：`actions/configure-pages@v5` 的 `enablement: true` 上游明确要求"`GITHUB_TOKEN` 以外的 token"（PAT 需 `repo` 或 Pages 写权限），本仓库不推荐为此多放一个长期 PAT。
